# ---
# title: "Translate Minnesota Natonal Forest Breeding Bird Monitoring Program data to make them ready for WT upload"
...
data_flat <- merge(s_data, pc_detection[,c(4:26)], by = "Visit-PKEY", all = TRUE)

# Fix column name using space in df
names(data_flat)<-str_replace_all(names(data_flat), c(" " = "_"))

# Translate df with species only and abundance
...
drive_upload(file.path(wd,"out", paste0(dataset_code, ".csv")),path=as_id(out_location$id),type = 'spreadsheet')
